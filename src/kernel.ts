...
if ( raw_memory.length === 0 ) {
    ...
    global.Memory = JSON.parse(raw_memory) as Memory; // TODO: Binary => Base<2^15> serialization
}
 ...
 */
const Task_Factories: Record<Task_Factory_Key, Task_Factory> = {};
...
export const run = ({
    ...
    if ( booting ) {
        ...
        Object.entries(tasks).forEach((arg: [string /*Task_ID*/, Task]) => {
            const task = arg[1];
    ...
    const skip = (task: Task, priority: PRIORITY, index: number) => {
        task.skips++;
