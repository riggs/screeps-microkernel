 ...
 */
const create_task = ({ priority, task_key, task_args, starvation_threshold, cost_average, parent, }) => {
 ...
 */
const kill_task = (id = current_task) => {
    const task = tasks[id];
    if (task === undefined)
 ...
 */
const get_task_ID = () => current_task;
/**
 ...
 */
const run = ({ alpha_min, alpha_decay, bucket_threshold, shutdown_cpu_estimate, sigma_range, log_level, logger } = {
    ...
    log_level: exports.LOG_LEVEL.WARN,
    logger: ((level, message) => console.log(`[${exports.LOG_LEVEL[level]}]`, message)),
}) => {
    /** Startup **/
    // Input Validation
    ...
    current_task = ROOT_TASK_ID;
    // Save kernel state
    if (kernel_alpha > alpha_min)
        stats.alpha = kernel_alpha * alpha_decay;
...
exports.get_task_ID = get_task_ID;
